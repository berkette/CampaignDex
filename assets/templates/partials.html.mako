<%def name="header()">
    <div id="header">
        ...
        <h1 onclick="go_to_home()">${attributes['campaign_name']}</h1>
        <button id="header_new_button" onclick="go_to_new()">New Page</button>
    </div>
</%def>

<%def name="sidebar()">
    <div id="sidebar">
        <button id="sidebar_home_button" onclick="go_to_home()">Home</button>
        <div id="path_links">
            <h3>Path</h3>
    ...
    % for subpage in attributes['subpages']:
                <li><a href="${subpage['path']}">${subpage['title']}</a></li>
    % endfor
            </ul>
        </div>
        <div id="quicklinks">
    ...
    % for quicklink in attributes['quicklinks']:
                <li><a href="${quicklink['path']}">${quicklink['title']}</a></li>
    % endfor
            </ul>
        </div>
    </div>
